...
const TaskCard: React.FC<TaskCardProps> = ({ task, onAction, showStatus = true }) => {
...
export const ClientDashboard: React.FC = () => {
  ...
  const [dashboard] = useState<ClientDashboardType>(() =>
    user ? generateClientDashboard(user.id) : MOCK_CLIENT_DASHBOARD
  );
  ...
  const handleTaskAction = (action: string, taskId: string) => {
    switch (action) {
      ...
      case 'track':
        ...
        break;
      case 'message':
        // Handle message messenger
        break;
      case 'reorder':
        // Handle reorder
        break;
      case 'rate':
        // Handle rate messenger
        break;
    }
  };
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          ...
          <p className="text-slate-500 text-sm">Manage your errands, track deliveries, and save time with templates.</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <div className="bg-white p-4 rounded-xl shadow-sm border border-slate-100">
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="text-xl font-bold text-slate-800">{dashboard.activeTasks.length}</div>
        ...
        <div className="bg-white p-4 rounded-xl shadow-sm border border-slate-100">
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="text-xl font-bold text-slate-800">{dashboard.completedTasksCount}</div>
        ...
        <div className="bg-white p-4 rounded-xl shadow-sm border border-slate-100">
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="text-xl font-bold text-slate-800">${dashboard.totalSpent.toFixed(2)}</div>
      ...
      <div className="bg-white rounded-xl shadow-sm border border-slate-100 overflow-hidden">
        <div className="flex border-b border-slate-100">
          ...
          <button
            ...
            className={`flex-1 px-6 py-3 text-sm font-medium transition-colors ${
              activeTab === 'history' ? 'bg-brand-50 text-brand-700 border-b-2 border-brand-500' : 'text-slate-600 hover:bg-slate-50'
            }`}
          >
            History ({dashboard.completedTasksCount})
          </button>
          <button
            onClick={() => setActiveTab('templates')}
            className={`flex-1 px-6 py-3 text-sm font-medium transition-colors ${
              activeTab === 'templates' ? 'bg-brand-50 text-brand-700 border-b-2 border-brand-500' : 'text-slate-600 hover:bg-slate-50'
            }`}
          >
            Templates ({dashboard.templates.length})
          </button>
          <button
            onClick={() => setActiveTab('addresses')}
            className={`flex-1 px-6 py-3 text-sm font-medium transition-colors ${
              activeTab === 'addresses' ? 'bg-brand-50 text-brand-700 border-b-2 border-brand-500' : 'text-slate-600 hover:bg-slate-50'
            }`}
          >
            Addresses ({dashboard.savedAddresses.length})
          </button>
        </div>
        ...
        <div className="p-6">
          {activeTab === 'active' && (
            <div className="space-y-6">
              ...
              <div className="space-y-4">
                <h3 className="font-semibold text-slate-800">Active & Upcoming Tasks</h3>
              ...
              {dashboard.recurringTasks.length > 0 && (
                <div className="space-y-4">
                  ...
                  {dashboard.recurringTasks.map(recurring => (
                    <div key={recurring.id} className="bg-slate-50 p-4 rounded-xl border border-slate-200">
                      <div className="flex items-center justify-between mb-2">
                        <h4 className="font-medium text-slate-800">Weekly Pharmacy Run</h4>
                        <div className="flex items-center gap-2">
                          <span className={`px-2 py-1 rounded-full text-xs font-medium ${
                            recurring.isActive ? 'bg-green-100 text-green-700' : 'bg-slate-100 text-slate-600'
                          }`}>
                      ...
                      <p className="text-xs text-slate-500">
                        Next run: {new Date(recurring.schedule.nextRun).toLocaleDateString()}
                      </p>
                    </div>
                  ))}
                </div>
          ...
          {activeTab === 'history' && (
            <div className="space-y-4">
              <div className="flex justify-between items-center mb-4">
                <h3 className="font-semibold text-slate-800">Task History</h3>
              ...
              {dashboard.completedTasks.slice(0, 5).map(task => (
                <TaskCard key={task.id} task={task} onAction={handleTaskAction} showStatus={false} />
              ))}
              {dashboard.completedTasks.length > 5 && (
                <div className="text-center pt-4">
                  <button className="text-brand-600 font-medium hover:underline">
                    View all {dashboard.completedTasksCount} completed tasks
                  </button>
                </div>
              )}
            </div>
          ...
          {activeTab === 'templates' && (
            <div className="space-y-4">
              <div className="flex justify-between items-center mb-4">
                <h3 className="font-semibold text-slate-800">Saved Templates</h3>
              ...
              {dashboard.templates.map(template => (
                <div key={template.id} className="bg-white p-4 rounded-xl border border-slate-200 hover:shadow-md transition-all">
                  <div className="flex items-start justify-between mb-3">
                    <div>
                  ...
                  <div className="flex items-center justify-between">
                    <div className="text-sm text-slate-500">
                      Used {template.usageCount} times • ~${template.estimatedPrice}
                    </div>
                    <div className="flex gap-2">
                      <button className="bg-brand-600 text-white px-3 py-1.5 rounded text-sm font-medium hover:bg-brand-700 transition-colors">
                        Use Template
                      </button>
                      <button className="bg-slate-100 text-slate-600 px-3 py-1.5 rounded text-sm font-medium hover:bg-slate-200 transition-colors">
                        Edit
                      </button>
                    </div>
                  </div>
          ...
          {activeTab === 'addresses' && (
            <div className="space-y-4">
              <div className="flex justify-between items-center mb-4">
                <h3 className="font-semibold text-slate-800">Saved Addresses</h3>
                <button className="bg-brand-600 text-white px-4 py-2 rounded-lg text-sm font-medium hover:bg-brand-700 transition-colors">
                  Add Address
                </button>
              </div>
              <div className="grid md:grid-cols-2 gap-4">
                {dashboard.savedAddresses.map(address => (
                  <div key={address.id} className="bg-white p-4 rounded-xl border border-slate-200 hover:shadow-md transition-all">
